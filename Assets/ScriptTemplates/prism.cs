...
public class prism : MonoBehaviour
{
    ...
    {
        Vector4 destination = movedUp ? startPropPos : targetPropPos;
        while (Vector4.Distance(propToMove.position, destination) > 0.01f)
        {
            propToMove.position = Vector4.MoveTowards(propToMove.position, destination, moveSpeed * Time.deltaTime);
            yield return null;
        }
